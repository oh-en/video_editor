...
path_to_probe = '/usr/local/bin/ffprobe'
...
def get_length(filename):
    probe = ffmpeg.probe(filename, cmd='/usr/local/bin/ffprobe')
    video_streams = [stream for stream in probe['streams'] if stream['codec_type'] == 'video']
        ...
        if os.path.isdir(video_path) and video_folder_name[0] != '.':
            ...
            if resolution.get() == 1:
                ...
                if outro_check.get():
                    ...
                    subprocess.call(command, shell=True)
                    outro_video_path_scaled = os.path.abspath(
                        'good_framerate_outro.mp4')  # updates the intro_video_path
            ...
            if resolution.get() == 2:
                ...
                if intro_check.get():
                    command = """{} -i "{}" -vf scale=3840:2160 -r 24 -c:v libx264 good_framerate_intro.mp4""".format(
                        path_to_ffmpeg, intro_video_path)
                    subprocess.call(command, shell=True)
                    intro_video_path_scaled = os.path.abspath(
                        'good_framerate_intro.mp4')  # updates the intro_video_path
            ...
            command = """{} -f concat -safe 0 -i "{}" -c copy videos_concat.mp4""".format(path_to_ffmpeg,
                                                                                          path_to_videotxt)
            subprocess.call(command, shell=True)
            ...
            #             path_to_image,width)
            #     subprocess.call(command, shell=True)
            #     command = """/usr/local/bin/ffmpeg -i output.mp4 -i scaled.png -filter_complex "overlay=0:H-h:enable='between(t,{},{})'" -codec:a copy final.mp4""".format(intro_time,video_time-outro_time)
            #     subprocess.call(command, shell=True)
