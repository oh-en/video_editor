...
def get_width(path_to_video):
    probe = ffmpeg.probe(path_to_video, cmd='/usr/local/bin/ffprobe')
    ...
    for video_path in os.listdir(video_path_head):
        ...
        if os.path.isdir(video_path) and video_folder_name[0] != '.':
            ...
            if resolution.get() == 1:
                ...
                if intro_check.get():
                    command = "/usr/local/bin/ffmpeg -i '{}' -vf scale=1920:1080 scaled_intro.mp4".format(intro_video_path)
                    subprocess.call(command, shell=True)
                    command = "/usr/local/bin/ffmpeg -i scaled_intro.mp4 -filter:v fps=fps=24 good_framerate_intro.mp4"
                    subprocess.call(command, shell=True)
                ...
                for i, file in enumerate(sorted(os.listdir(video_path))):
                    if os.path.isdir(video_path) and video_folder_name[0] != '.' and file[0]!= '.':
                        command = "/usr/local/bin/ffmpeg -i '{}' -vf scale=1920:1080 temp_videos/vid_{}.mp4".format(
                            os.path.abspath(os.path.join(video_path, file)),
                            i)  # converts every video to the resolution, adds it to temp file
                        subprocess.call(command, shell=True)
                        command = "/usr/local/bin/ffmpeg -i temp_videos/vid_{}.mp4 -filter:v fps=fps=24 temp_videos/video_{}.mp4".format(i, i)
                        subprocess.call(command, shell=True)
                        os.remove('temp_videos/vid_{}.mp4'.format(i))
                video_path = os.path.abspath('temp_videos/')  # update the video path to the temp videos
                if outro_check.get():
                    command = "/usr/local/bin/ffmpeg -i '{}' -vf scale=1920:1080 scaled_outro.mp4".format(outro_video_path)
                    subprocess.call(command, shell=True)
                    command = "/usr/local/bin/ffmpeg -i scaled_outro.mp4 -filter:v fps=fps=24 good_framerate_outro.mp4"
                    subprocess.call(command, shell=True)
            ...
            if resolution.get() == 2:
                ...
                # converts them to 4k
                if intro_check.get():
                    command = "/usr/local/bin/ffmpeg -i '{}' -vf scale=3840:2160 scaled_intro.mp4".format(intro_video_path)
                    subprocess.call(command, shell=True)
                    command = "/usr/local/bin/ffmpeg -i scaled_intro.mp4 -filter:v fps=fps=24 good_framerate_intro.mp4"
                    subprocess.call(command, shell=True)
                    intro_video_path_scaled = os.path.abspath('good_framerate_intro.mp4')  # updates the intro_video_path
                ...
                for i, file in enumerate(sorted(os.listdir(video_path))):
                    if os.path.isdir(video_path) and video_folder_name[0] != '.' and file[0]!= '.':
                        command = "/usr/local/bin/ffmpeg -i '{}' -vf scale=3840:2160 temp_videos/vid_{}.mp4".format(
                            os.path.abspath(os.path.join(video_path, file)),
                            i)  # converts every video to the resolution, adds it to temp file
                        subprocess.call(command, shell=True)
                        command = "/usr/local/bin/ffmpeg -i temp_videos/vid_{}.mp4 -filter:v fps=fps=24 temp_videos/video_{}.mp4".format(i, i)
                ...
                video_path = os.path.abspath('temp_videos/')  # update the video path to the temp videos
                if outro_check.get():
                    command = "/usr/local/bin/ffmpeg -i '{}' -vf scale=3840:2160 scaled_outro.mp4".format(outro_video_path)
                    subprocess.call(command, shell=True)
                    command = "/usr/local/bin/ffmpeg -i scaled_outro.mp4 -filter:v fps=fps=24 good_framerate_outro.mp4"
                    subprocess.call(command, shell=True)
                    outro_video_path_scaled = os.path.abspath('good_framerate_outro.mp4')  # updates the intro_video_path
            ...
            ffmpeg.input(path_to_videotxt, format='concat', safe=0).output('videos_concat.mp4', c='copy').run(
                    cmd='/usr/local/bin/ffmpeg',overwrite_output=True)
            ...
            if music_check.get():
                ...
                command = "/usr/local/bin/ffmpeg -i videos_concat.mp4 -i audio_concat.mp3 -map 0:v -map 1:a -c:v copy -shortest output.mp4"
                subprocess.call(command, shell=True)
            ...
            if overlay_check.get():
                width = get_width('output.mp4') # gets the width of the image
                print(time.strftime("%H:%M:%S", time.localtime()), ': Adding image overlay...')
                command = """/usr/local/bin/ffmpeg -i '{}' -y -v quiet -vf scale={}:-1 scaled.png""".format(
                        path_to_image,width)
                subprocess.call(command, shell=True)
                command = """/usr/local/bin/ffmpeg -i output.mp4 -i '{}' -filter_complex "overlay=0:H-h" -codec:a copy final.mp4""".format('scaled.png')
                subprocess.call(command, shell=True)
